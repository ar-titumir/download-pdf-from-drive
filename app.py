...
def run_scraper(URL, IMG_DIR, PDF_PATH, status_label):
    ...
    pdf.output(PDF_PATH)
    status_label.config(text=f"Saved: {PDF_PATH}")
...
def start_process():
    ...
    if input_path.startswith("http"):
        if not pdf_name:
            pdf_name = timestamp_name()

        pdf_path = os.path.join(pdf_dir, pdf_name)
        img_dir = os.path.join(pdf_dir, "temp_imgs")

        status_label.config(text="Processing single file...")
        root.update()

        run_scraper(input_path, img_dir, pdf_path, status_label)
    ...
    # ---------- CASE 2: text file ----------
        ...
        for line in lines:
            ...
            status_label.config(text=f"Processing: {name}")
            root.update()

            run_scraper(url, img_dir, pdf_path, status_label)
...
# ---------------- WINDOW ----------------
root = tk.Tk()
root.title("PDF file maker from Drive link")
